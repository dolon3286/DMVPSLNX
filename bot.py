...
# --- UPDATED: Connect VPN Function with Real-Time Error Checking ---
async def connect_vpn(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status_msg = await update.message.reply_text(f"🔌 Connecting VPN...\nConfig: {os.path.basename(config_path)}\nServer: {server}")
    
    try:
        command = get_vpn_start_command(config_path)
        if os.path.splitext(config_path)[1].lower() == '.ovpn':
            # Run asynchronously so we can capture stdout without freezing the bot
            ...
            async def monitor_openvpn_logs():
                log_history = ""
                connected = False
                while True:
                    ...
                    if "Initialization Sequence Completed" in decoded_line:
                        connected = True
                        break
                    # Catch the exact reasons OpenVPN hangs
                    if "Auth" in decoded_line and "failed" in decoded_line.lower():
                        break
                    if "Enter Auth Username" in decoded_line:
                        break
                    if "Cannot open TUN/TAP dev" in decoded_line:
                        break
                return connected, log_history

            try:
                # Wait up to 15 seconds for OpenVPN to announce connection complete
                success, output_log = await asyncio.wait_for(monitor_openvpn_logs(), timeout=15.0)
                if not success:
                    process.terminate()
                    raise Exception(f"VPN encountered an error or needs manual authentication:\n\n{output_log[-800:]}")
            except asyncio.TimeoutError:
                process.terminate()
                raise Exception("VPN connection timed out waiting for 'Initialization Sequence Completed'. It may be stuck or failing.")
        ...
        save_vpn_config(config)
        info = format_ip_info(get_public_ip_info())
        await status_msg.edit_text(f"✅ VPN successfully connected.\nServer: {server}\n{info}")
        
    except Exception as exc:
        VPN_STATE['process'] = None
        config['enabled'] = False
        save_vpn_config(config)
        await status_msg.edit_text(f"❌ VPN connection failed:\n`{str(exc)[:900]}`", parse_mode=ParseMode.MARKDOWN)
...
async def disconnect_vpn(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if process:
        ...
        if hasattr(process, 'poll') and process.poll() is None:
            is_running = True
        elif hasattr(process, 'returncode') and process.returncode is None:
            is_running = True

        if is_running:
            process.terminate()
            try:
                if hasattr(process, 'wait') and asyncio.iscoroutinefunction(process.wait):
